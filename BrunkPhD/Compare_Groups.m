...
% AllOpto = load('Output_Input_HighP_7post_Data_Threshold_25_Zscore_0_binned_1.mat');
HighP = load('Output_Input_HighP_7post_Data_Threshold_25_Zscore_0_binned_1.mat');
Med = load('Output_Input_MediumP_Data_Threshold_25_Zscore_0_binned_1.mat');
Low = load('Output_Input_LowP_Data_Threshold_25_Zscore_0_binned_1.mat');
Control = load('Output_Input_Control_7post_n7_Data_Threshold_25_Zscore_0_binned_1.mat');
YFP = load('Output_Input_YFP_7post_Data_Threshold_25_Zscore_0_binned_1.mat');
% YFP = load('Output_Input_Control_7post_n7_Data_Threshold_25_Zscore_0_binned_1.mat');
Groups = {'Control','YFP','AllOpto','HighP','Med','Low'};
...
para = {'SinkPeakAmp','Full_RMS_AVREC','Early_RMS_AVREC',...
    'Late_RMS_AVREC','Full_RMS_RELRES','Early_RMS_RELRES','Late_RMS_RELRES',...
    };
...
for i1 = 1:length(para)
    ...
    for i2 = 1:length(sorting)    
        for i3 = 1:Entries
            ...
            h= figure('Name',[para{i1} '_' sorting{i2} '_' CondName ],...
                'PaperType','a4letter',...
...
%             keyboard
            t = text(1,23,[para{i1} ' ' sorting{i2} ' ' CondName]);
            s = t.FontSize;
            t.FontSize = 12;
            
            cd (FigFolder)
            saveas(gcf,[para{i1} '_' sorting{i2} '_' CondName],'pdf');            
            savefig(gcf,[para{i1} '_' sorting{i2} '_' CondName],'compact');
            close all
