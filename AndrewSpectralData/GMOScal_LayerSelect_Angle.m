function GMOScal_LayerSelect_Angle(layer, rel2BFin)
...
awake2 = angle(awake2);
%awakeFig = figure;
completeScaleFig = figure('Name',['AP Complete Scale ' layer ' ' num2str(rel2BFin)],'Position',[-1070 500 1065 400]); 
...
savefig(['AP Complete Scale ' layer ' ' num2str(rel2BFin)])
saveas(gcf, ['AP Complete Scale ' layer ' ' num2str(rel2BFin) '.pdf'])
%% Scaled for awake condition (considers also Musc condition)

%close all
[X,Y]=meshgrid(wtTable.freq{1},params.startTime*1000:(params.limit-201));
awake2 = angle(awake2);
anestScaleFig = figure('Name', ['AP Awake Scale ' layer ' ' num2str(rel2BFin)],'Position',[-1070 500 1065 400]); 
awakeFig = subplot(131);
surf(Y,X,squeeze(mean(awake2,1))','EdgeColor','None'); view(2);
set(gca,'YScale','log'); title('Awake')
colorbar
...
set(h, 'PaperOrientation', 'landscape');
set(h, 'PaperUnits', 'centimeters');
savefig(['AP Awake Scale ' layer ' ' num2str(rel2BFin)])
saveas(gcf, ['AP Awake Scale ' layer ' ' num2str(rel2BFin) '.pdf'])
%% Log scale
%close all
[X,Y]=meshgrid(wtTable.freq{1},params.startTime*1000:(params.limit-201));
logScaleFig = figure('Name', ['AP Log Scale ' layer ' ' num2str(rel2BFin)],'Position',[-1070 500 1065 400]); 
awakeFig = subplot(131);
...
set(h, 'PaperOrientation', 'landscape');
set(h, 'PaperUnits', 'centimeters');
savefig(['AP Log Scale ' layer ' ' num2str(rel2BFin)])
saveas(gcf, ['AP Log Scale ' layer ' ' num2str(rel2BFin) '.pdf'])
%% Save
