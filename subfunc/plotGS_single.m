function plotGS_single(Data,Data_GSBased,names,SINKs,para,zscore,nThresh,bin,mirror,home,CurAn,BF_Pos)
...
SA =[];
    for i2 = 1:length(para)
        ...
        for i3 =1: length(names)
            ...
            for i4 = 1:Entries
                dummy =[];
                for i5 =1:length(Data_GSBased)
                    if ~isempty(Data(i5).(names{i3}))
                        if isstruct(Data_GSBased(1).(para{i2}))
                            try
                                dummy = vertcat(dummy, Data_GSBased(i5).(para{i2}).(SINKs{i4})(i3,:));
                            catch
                                dummy = vertcat(dummy,nan(1, size(dummy,2)));
                            end
                        else
                            try
                                dummy = vertcat(dummy, Data_GSBased(i5).(para{i2})(i3,:));
                            catch
                                dummy = vertcat(dummy,nan(1, size(dummy,2)));
                            end
                        end
    ...
    for i2 = 1:length(names)
        ...
        for i3 = 1:length(para)
            h=figure('units','normalized','outerposition',[0 0 1 1],'Name',[CurAn(1:end-4) '_' names{i2}...
                '_GS based TempAnal_' para{i3} '_Thresh_' num2str(nThresh) '_zscored_' num2str(zscore) '_bin_' num2str(bin) '_mirror_' num2str(mirror)]);
            ...
            savefig(h,[CurAn(1:end-4) '_' names{i2}...
                '_GS_tempAnal_' para{i3} '_Thresh_' num2str(nThresh)...
