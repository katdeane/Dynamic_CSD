...
%     T = table([Kpeakamp';Apeakamp';Mpeakamp'; repmat({newTicks{ifreq}}, 1, grpsz)';repmat({Group{1}},1,grpsz)']);%,'RowNames',{'PeakAmp','PeakLat','Freq','Group'}
    ...
    plottune.totalmean = horzcat(plottune.totalmean, Ktotalvalue, Atotalvalue, Mtotalvalue);
    
    plottune.peakamp_norm = horzcat(plottune.peakamp_norm, Kpeakamp_norm, Apeakamp_norm, Mpeakamp_norm);
    plottune.smallmean_norm = horzcat(plottune.smallmean_norm, Kpeakvalue_norm, Apeakvalue_norm, Mpeakvalue_norm);
...
cd(homedir); cd figs; mkdir('SnglAvRec Tuning'); cd('SnglAvRec Tuning')
