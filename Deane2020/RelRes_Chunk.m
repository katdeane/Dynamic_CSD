...
% this code will eventually take Relres data, bin it into 10ms chunks, take 
...
BFdis = -2;      % = 0 if BF, -1 for 1 below BF, etc.
...
for iAn = 1:length(K_Animal)
    ...
    if BF+BFdis <= 0
        ...
        curRR = Ketamine.(K_Animal{iAn}).RELRES_raw{1,BF+BFdis}';
    end
    
    % preallocate lists to store from loop
    chunking = 1:chunksize:size(curRR,2); %numbers to iterate (every 10ms)
    relresRMS_Avg = zeros(size(chunking)); %list for RMS output (1:60)
    relresCSD_Avg = curRR(1:600); %list for relres output (1:600)
    
    % loop through each trial in steps of 10ms to calc relres/rms of relres
    for ichunk = chunking
        chunkRR = curRR(:,ichunk:ichunk+9);
        relresRMS_Avg(chunking == ichunk) = rms(chunkRR);
    end
    ...
    if isempty(p)
        ...
        RELpeakamp_Avg(itrial) = NaN;
        RELpeaklat_Avg(itrial) = NaN;
        RELmean_Avg(itrial) = NaN;
    else
    ...
    if BF+BFdis <= 0
        cursnglRR = NaN([600 1 50]);
    else
        cursnglRR = Ketamine.(K_Animal{iAn}).SingleTrial_RelRes_raw{1,BF+BFdis};
    end
    
    % preallocate lists
    trials = 1:size(cursnglRR,3);
    ...
    for itrial = trials
        ...
        trialRR = cursnglRR(:,:,itrial)';
        
        % preallocate lists to store from loop
        chunking = 1:chunksize:size(cursnglRR,1); %numbers to iterate (every 10ms)
        relresRMS = zeros(size(chunking)); %list for RMS output (1:60)
        relresCSD = trialRR(1:600); 
        
        % loop through each trial in steps of 10ms to calc relres/rms of relres
        for ichunk = chunking
            chunkRR = trialRR(:,ichunk:ichunk+9);
            relresRMS(chunking == ichunk) = rms(chunkRR);
        end
        ...
        if isempty(p)
            ...
            RMSmean(itrial) = NaN;
            RELpeakamp(itrial) = NaN;
            RELpeaklat(itrial) = NaN;
            RELmean(itrial) = NaN;
        else
            [~, maxInd]=max(p); %which peak is most prominent
...
for iAn = 1:length(A_Animal)
    ...
    if BF+BFdis <= 0
        ...
        curRR = Awake.(A_Animal{iAn}).RELRES_raw{1,BF+BFdis}';
    end
    ...
    if BF+BFdis <= 0
        ...
        cursnglRR = Awake.(A_Animal{iAn}).SingleTrial_RelRes_raw{1,BF+BFdis};
    end
...
for iAn = 1:length(M_Animal)
    ...
    if BF+BFdis <= 0
        ...
        curRR = Muscimol.(M_Animal{iAn}).RELRES_raw{1,BF+BFdis}';
    end
    ...
    if BF+BFdis <= 0
        ...
        cursnglRR = Muscimol.(M_Animal{iAn}).SingleTrial_RelRes_raw{1,BF+BFdis};
    end
...
ARmsMean = horzcat(ARmsMean{:,1}, NaN)';
