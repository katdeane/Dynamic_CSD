...
which = 2;                  % 1 = Anesthetized Chronic split
                            % 2 = Anesthetized split
                            % 3 = Anesthetized vs Anesthetized Chronic
                            % 4 = Awake Split
...
if which == 1 || which == 4 % current t value threshold ONLY good for this df
    df = length(first_names) - 2; % get true df to make sure it matches
    if df == 7
        t_thresh = 2.365; % http://www.ttable.org/
    else
        error('You need to change your tvalue threshold! Check this link: http://www.ttable.org/ (df is group size 1 + group size 2 - 2)')
    end
elseif which == 2
    df = length(first_names) - 2; % get true df to make sure it matches
    if df == 9
...
if which == 3
    ...
    for iA = 1:floor(length(first_names)/2) %for each animal
        BF = find((First_Group.(first_names{iA}).GS_BF) == (First_Group.(first_names{iA}).Frqz')); %get the BF Position
    ...
    for iA = floor(length(first_names)/2) + 1:floor(length(first_names)/2) + ceil(length(first_names)/2) %for each animal
        BF = find((First_Group.(first_names{iA}).GS_BF) == (First_Group.(first_names{iA}).Frqz')); %get the BF Position
        
        for itick = -3:ticklen-4 %through each freqency around charted BF
            if BF+itick > 0 && BF+itick <= length(First_Group.(first_names{iA}).CSD)
                second_cont{iA - floor(length(first_names)/2),itick+4} = First_Group.(first_names{iA}).CSD{BF+itick}(1:28,1:600);
            else
                second_cont{iA - floor(length(first_names)/2),itick+4} = NaN(28,600);
            end
        end
    end
end
...
elseif which == 2 
    ...
    for itick = 1:length(Ticknames) %creates 1x7 CSDs with 4 being the BF
        ...
        cond2cat = cat(3,second_cont{1,itick},second_cont{2,itick},second_cont{3,itick},second_cont{4,itick}...
            ,second_cont{5,itick},second_cont{6,itick}); %6 animals
...
if which == 3
    ...
    for itick = 1:ticklen
        ...
        obs_t{itick} = (obs1_means{itick} - obs2_means{itick})./...
            sqrt((obs1_std{itick}.^2/floor(length(first_names)/2)) + (obs2_std{itick}.^2/ceil(length(first_names)/2)));
        obs_clusters{itick} = abs(obs_t{itick});
        obs_clusters{itick}(obs_clusters{itick} < t_thresh) = NaN;
        obs_clusters{itick}(obs_clusters{itick} >= t_thresh) = 1;
...
for iperm = 1:nperms
    ...
    if which == 1 || which == 4
        ...
        for itick = 1:ticklen %creates 1x7 CSDs with 4 being the BF
            ...
            allcondcat = cat(3,first_cont{1,itick},first_cont{2,itick},first_cont{3,itick},first_cont{4,itick}...
                ,second_cont{1,itick},second_cont{2,itick},second_cont{3,itick},second_cont{4,itick},...
                second_cont{5,itick});
            ...
            cond2perm = cat(3,allcondcat(:,:,Pv(5)),allcondcat(:,:,Pv(6)),allcondcat(:,:,Pv(7)),allcondcat(:,:,Pv(8))...
                ,allcondcat(:,:,Pv(9)));
    ...
    elseif which == 2 
        ...
        for itick = 1:ticklen %creates 1x7 CSDs with 4 being the BF
            ...
            allcondcat = cat(3,first_cont{1,itick},first_cont{2,itick},first_cont{3,itick},first_cont{4,itick}...
                ,first_cont{5,itick},second_cont{1,itick},second_cont{2,itick},second_cont{3,itick},second_cont{4,itick},...
                second_cont{5,itick},second_cont{6,itick});
            ...
            cond2perm = cat(3,allcondcat(:,:,Pv(6)),allcondcat(:,:,Pv(7)),allcondcat(:,:,Pv(8)),allcondcat(:,:,Pv(9))...
                ,allcondcat(:,:,Pv(10)),allcondcat(:,:,Pv(11)));
    ...
    for itick = 1:ticklen
        if which == 1 || which == 2 || which == 4
            perm_t{itick} = (perm1_means{itick} - perm2_means{itick})./...
                sqrt((perm1_std{itick}.^2/floor(length(first_names)/2)) + (perm2_std{itick}.^2/floor(length(first_names))));
...
[sig_mass,sig_max, pVal] = Perm_makegramm(nperms,obs_clustermass, permstruct.clustermass, ...
...
if which == 1
    save N_Split_permstats.mat sig_mass sig_max IIE_sig_mass IIE_sig_max IVE_sig_mass IVE_sig_max ...
...
elseif which == 2
    save K_Split_permstats.mat sig_mass sig_max IIE_sig_mass IIE_sig_max IVE_sig_mass IVE_sig_max ...
        VbE_sig_mass VbE_sig_max VIaE_sig_mass VIaE_sig_max IIL_sig_mass IIL_sig_max IVL_sig_mass ...
        IVL_sig_max VbL_sig_mass VbL_sig_max VIaL_sig_mass VIaL_sig_max obs_tstat obs_H ...
        IIE_pVal IIL_pVal IVE_pVal IVL_pVal VbL_pVal VbE_pVal VIaE_pVal VIaL_pVal pVal
elseif which == 3
    save NvK_permstats.mat sig_mass sig_max IIE_sig_mass IIE_sig_max IVE_sig_mass IVE_sig_max ...
        VbE_sig_mass VbE_sig_max VIaE_sig_mass VIaE_sig_max IIL_sig_mass IIL_sig_max IVL_sig_mass ...
        IVL_sig_max VbL_sig_mass VbL_sig_max VIaL_sig_mass VIaL_sig_max obs_tstat obs_H...
        IIE_pVal IIL_pVal IVE_pVal IVL_pVal VbL_pVal VbE_pVal VIaE_pVal VIaL_pVal pVal
elseif which == 4
    save A_Split_permstats.mat sig_mass sig_max IIE_sig_mass IIE_sig_max IVE_sig_mass IVE_sig_max ...
        VbE_sig_mass VbE_sig_max VIaE_sig_mass VIaE_sig_max IIL_sig_mass IIL_sig_max IVL_sig_mass ...
        IVL_sig_max VbL_sig_mass VbL_sig_max VIaL_sig_mass VIaL_sig_max obs_tstat obs_H ...
        IIE_pVal IIL_pVal IVE_pVal IVL_pVal VbL_pVal VbE_pVal VIaE_pVal VIaL_pVal pVal
end
