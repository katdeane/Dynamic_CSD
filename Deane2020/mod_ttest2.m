function [h,p,ci,stats] = mod_ttest2(x,y,varargin)
% based on the ttest2 built-in matlab function - the difference being that as 
% I already had 2 averaged matrices so I removed the need to mean the input
% again. The rest of matlab's documentation on this function is as follows:
...
fix = (difference~=0) & ...                                     % non-zero
    (abs(difference) < sqrtn.*100.*max(eps(x),eps(y))); % but small                                                                                 -
if any(fix(:))
...
% if requested.
dfemat = repmat(dfe,[nx,1]);
if tail == 0 % two-tailed test
    p = 2 * tcdf(-abs(ratio),dfemat);
    if nargout > 2
        spread = tinv(1 - alpha ./ 2, dfemat) .* se;
        ci = cat(dim, difference-spread, difference+spread);
    end
elseif tail == 1 % right one-tailed test
    p = tcdf(-ratio,dfemat);
    if nargout > 2
        spread = tinv(1 - alpha, dfemat) .* se;
