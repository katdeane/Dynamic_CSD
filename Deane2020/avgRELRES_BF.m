...
%  plots for the averaged RELRES as output.
...
for i1 = 1:entries
    ...
    mkdir(['AvgRelres_' input(i1).name(1:end-9)]); cd(['AvgRelres_' input(i1).name(1:end-9)]);
   ...
   for i2 = 1:length(names)
       ...
       relres.(animal).BF =  Data.(animal).RELRES_raw{1,BF}';
       ...
       %-1
       try 
           relres.(animal).min_one = Data.(animal).RELRES_raw{1,(BF-1)}';  
       catch
       ...
       %-2
       try 
           relres.(animal).min_two = Data.(animal).RELRES_raw{1,(BF-2)}';  
       catch
       ...
       %-3
       try 
           relres.(animal).min_three = Data.(animal).RELRES_raw{1,(BF-3)}';  
       catch
       ...
       %+1
       try 
           relres.(animal).plus_one = Data.(animal).RELRES_raw{1,(BF+1)}';  
       catch
       ...
       %+2
       try 
           relres.(animal).plus_two = Data.(animal).RELRES_raw{1,(BF+2)}';  
       catch
       ...
       %+3
       try 
           relres.(animal).plus_three = Data.(animal).RELRES_raw{1,(BF+3)}';  
       catch
...
%% Gramm Plots
cd(home); cd figs; %opens figure folder to store future images
mkdir('Gramm Plots AvgRelres'); cd('Gramm Plots AvgRelres');
...
for iticks = 1:length(Ticks)
    ...
    g.axe_property('ylim',[-0.4 0.3],'xlim',[0 600])
...
levels = [2,1];
cd(home); cd DATA
mkdir('Stats avgRelRes'); cd('Stats avgRelRes')
...
for iticks = 1:length(Ticks)
    ...
    for istack = 1:ANsize
        [latency, ampl, meanOfdata] = iGetPeakData_relres(plotdata.(Ticks{iticks})(winstart(istack):winend(istack)));
        if isnan(meanOfdata)
...
cd(home); cd figs; mkdir('AvgRelRes Tuning'); cd('AvgRelRes Tuning')
...
%     g(1,1).axe_property('YLim',[0 0.2]);
else
    g(1,1).set_names('x','Tone','y','mV*mm²','color','Group');
%     g(1,1).axe_property('YLim',[-0.15 0.1]);
end
...
g.set_title('RelRes Peak Latency against Amp');
g.draw();
...
g=gramm('x',AVRECdata.peaklate,'y',plotdata.peaklate, 'color', plotdata.tgroup);
g.facet_grid(plotdata.frqz,[]);
...
g.set_title('Avrec against Relres Peak Latency');
g.draw();
g.export('file_name','Avrec against Relres Peak Latency', 'file_type','pdf');
g.export('file_name','Avrec against Relres Peak Latency', 'file_type','png');
close all;
...
g=gramm('x',AVRECdata.peakamp,'y',plotdata.peakamp, 'color', plotdata.tgroup);
g.facet_grid(plotdata.frqz,[]);
g.geom_point(); %'alpha',0.5
% g.axe_property('YLim',[0 0.015]);
g.set_color_options('map','matlab');
...
g.set_title('Avrec against Relres Peak Amplitude');
g.draw();
g.export('file_name','Avrec against Relres Peak Amplitude', 'file_type','pdf');
g.export('file_name','Avrec against Relres Peak Amplitude', 'file_type','png');
close all;